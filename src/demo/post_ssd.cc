...
*/
void SSDPostProcessor::transpose_and_copy(float* boxes_float, float* clses_float,
...
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[0]; i++) {
        boxes_float[indexes_boxes[0] + i] = result[10][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[1]; i++) {
        boxes_float[indexes_boxes[1] + i] = result[0][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[2]; i++) {
        boxes_float[indexes_boxes[2] + i] = result[2][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[3]; i++) {
        boxes_float[indexes_boxes[3] + i] = result[4][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[4]; i++) {
        boxes_float[indexes_boxes[4] + i] = result[6][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_boxes[5]; i++) {
        boxes_float[indexes_boxes[5] + i] = result[8][i];
    }

#pragma omp parallel for
    for (int i = 0; i < lengths_clses[0]; i++) {
        clses_float[indexes_clses[0] + i] = result[11][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[1]; i++) {
        clses_float[indexes_clses[1] + i] = result[1][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[2]; i++) {
        clses_float[indexes_clses[2] + i] = result[3][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[3]; i++) {
        clses_float[indexes_clses[3] + i] = result[5][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[4]; i++) {
        clses_float[indexes_clses[4] + i] = result[7][i];
    }
#pragma omp parallel for
    for (int i = 0; i < lengths_clses[5]; i++) {
        clses_float[indexes_clses[5] + i] = result[9][i];
    }
}
...
*/
float SSDPostProcessor::area(float xmin, float ymin, float xmax, float ymax) {
...
*/
float SSDPostProcessor::calculate_iou(array<float, 4> box1, array<float, 4> box2) {
...
/*
    Prior Generation.
*/
float* SSDPostProcessor::prior_generation() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (i != 0) all_sizes.push_back(make_pair(sk3, sk3));
...
/*
    Decode models regression outputs, change from offset(dy,dx,dh,dw) form to
   relative(0-1) coordinates in corner form(xmin, ymin, xmax, ymax)
*/
float* SSDPostProcessor::decode(float* locations, float* priors) {
    ...
    for (int i = 0; i < num_boxes; i++) {
        ...
        float pred_ctr_x =
            locations[idx + 1] * weights[1] * priors[idx + 2] + priors[idx];
        float pred_ctr_y =
            locations[idx] * weights[0] * priors[idx + 3] + priors[idx + 1];
...
/*
    This function decodes boxes, filters(mask) inference results with score_thresold = 0.3
   and applies NMS(nms_threshold = 0.5)
*/
int SSDPostProcessor::filter_results(float* init_scores, float* init_boxes,
                                     vector<float>& boxes_final,
                                     vector<float>& classes_final,
                                     vector<float>& scores_final, float nms_threshold) {
    int length = 1917;
    int num_output = 0;
...
#pragma omp parallel for num_threads(mOpenmpThreadCount), default(none),        \
    shared(output_scores, output_labels, output_boxes, init_scores, init_boxes, \
           nms_threshold, length)
    for (int i = 1; i < 91; i++) {
        ...
        for (int j = 0; j < length; j++) {
            ...
            if (temp_score > 0.3) {
                // init_boxes and init_scores are model detection and classification
                // outputs in one-dimensional array in Python implementation they have
                // following shapes - (1, 1917, 4) and (1, 1917, 91) respectively thus,
                // init_boxes[j * 4 + k] stands for j'th row and k'th column of Python
                // boxes array similarly, init_scores[j * 9 + i] stands for j'th row and
                // i'th column of Python scores array
        ...
        for (int idx = 0; idx < _size; idx++) {
            ...
            if (masked_scores[idx].first > 0) {
                ...
                array<float, 4> max_box = {
                    init_boxes[index_max_score + 0], init_boxes[index_max_score + 1],
                    init_boxes[index_max_score + 2], init_boxes[index_max_score + 3]};
                ...
                for (int j = idx + 1; j < _size; j++) {
                    if (masked_scores[j].first > 0) {
                        ...
                        array<float, 4> temp_box = {
                            init_boxes[index + 0], init_boxes[index + 1],
                            init_boxes[index + 2], init_boxes[index + 3]};
    ...
    for (int i = 0; i < mOpenmpThreadCount; i++) {
...
int SSDPostProcessor::postprocessing(vector<vector<float>>& result, vector<float>& boxes,
                                     vector<float>& classes, vector<float>& scores) {
    constexpr int boxes_length = 7668;
    constexpr int clses_length = 174447;
    ...
    transpose_and_copy(boxes_float, clses_float, result);
...
#pragma omp parallel for
    ...
    int num_output =
        filter_results(clses_float, boxes_float, boxes, classes, scores, 0.5);

    return num_output;
}

SSDPostProcessor::SSDPostProcessor() {
    mThread = thread(&SSDPostProcessor::worker, this);
}
...
void SSDPostProcessor::reshape(int loop, int total_height, int total_channel,
...
void SSDPostProcessor::worker() {
    omp_set_dynamic(0);
    omp_set_num_threads(mOpenmpThreadCount);
    ...
    while (!destroyed) {
        ...
        num_output = postprocessing(
            // k.clses_float, k.boxes_float,
            k.result, k.boxes, k.classes, k.scores);
        auto end = chrono::system_clock::now();
        auto elapsed = end - start;

#ifdef DEBUG
        cout << "Postprocessor: Main postprocessing done in " << elapsed.count()
             << "(ns)." << endl;
        cout << "Postprocessor: sizeof boxes " << k.boxes.size() << " classes "
             << k.classes.size() << endl;
#endif
...
uint64_t SSDPostProcessor::enqueue(
    // float *clses_float, float *boxes_float,
    std::vector<std::vector<float>>& result, vector<float>& boxes, vector<float>& classes,
    vector<float>& scores) {
#ifdef DEBUG
...

    {
        ...
        mQueueIn.push({// ++ticket, clses_float, boxes_float, boxes, classes, scores
                       ++ticket, result, boxes, classes, scores});
        ticket_save = ticket;
...
void SSDPostProcessor::receive(uint64_t receipt_no) {
    while (!destroyed) {
#ifdef DEBUG
