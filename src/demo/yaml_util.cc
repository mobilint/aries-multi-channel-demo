...
void generateDefaultModelSettingYAML(const std::string& path) {
    ...
    model_setting_node[0]["mxq_path"] = "../mxq/yolo26s-pose.mxq";
    model_setting_node[0]["subconfig"]["input"] = "float32";
    model_setting_node[0]["subconfig"]["post"] = "anchorless";
    model_setting_node[0]["core_id"].push_back(core_id_node[0]);
    model_setting_node[0]["core_id"].push_back(core_id_node[1]);

    model_setting_node[1]["model_type"] = "FACE";
    model_setting_node[1]["mxq_path"] = "../mxq/yolo11s-face.mxq";
    model_setting_node[1]["subconfig"]["input"] = "float32";
    model_setting_node[1]["subconfig"]["post"] = "anchor";
    model_setting_node[1]["core_id"].push_back(core_id_node[2]);
    model_setting_node[1]["core_id"].push_back(core_id_node[3]);

    model_setting_node[2]["model_type"] = "SEGMENTATION";
    model_setting_node[2]["mxq_path"] = "../mxq/yolo26s-seg.mxq";
    model_setting_node[2]["subconfig"]["input"] = "float32";
    model_setting_node[2]["subconfig"]["post"] = "anchorless";
    model_setting_node[2]["core_id"].push_back(core_id_node[4]);
    model_setting_node[2]["core_id"].push_back(core_id_node[5]);

    model_setting_node[3]["model_type"] = "OBJECT";
    model_setting_node[3]["mxq_path"] = "../mxq/yolo26s.mxq";
    model_setting_node[3]["subconfig"]["input"] = "float32";
    model_setting_node[3]["subconfig"]["post"] = "anchorless";
...
std::vector<ModelSetting> Demo::loadModelSettingYAML(const std::string& path,
    ...
    model_type_map["FACE"] = ModelType::FACE;
    model_type_map["POSE"] = ModelType::POSE;
    model_type_map["OBJECT"] = ModelType::OBJECT;
    model_type_map["SEGMENTATION"] = ModelType::SEGMENTATION;

    std::map<std::string, InputDataType> input_type_map;
    input_type_map["uint8"] = InputDataType::UINT8;
    input_type_map["float32"] = InputDataType::FLOAT32;

    std::map<std::string, PostProcessType> post_type_map;
    post_type_map["anchor"] = PostProcessType::ANCHOR;
    post_type_map["anchorless"] = PostProcessType::ANCHORLESS;
    post_type_map["dflfree"] = PostProcessType::DFLFREE;
    post_type_map["nmsfree"] = PostProcessType::NMSFREE;
    ...
    for (int i = 0; i < ms_node.size(); i++) {
        ...
        if (model_type_map.find(model_type_str) == model_type_map.end()) {
            throw std::invalid_argument("Unsupported model_type in ModelSetting.yaml: " +
                                        model_type_str);
        }
        ms.model_type = model_type_map[model_type_str];
        ms.mxq_path = ms_node[i]["mxq_path"].as<std::string>();

        if (ms.model_type == ModelType::SSD) {
            ms.input_type = InputDataType::FLOAT32;
            ms.post_type = PostProcessType::ANCHORLESS;
        } else if (ms.model_type == ModelType::STYLENET) {
            ms.input_type = InputDataType::FLOAT32;
            ms.post_type = PostProcessType::ANCHORLESS;
        } else {
            YAML::Node subconfig_node = ms_node[i]["subconfig"];
            if (!subconfig_node) {
                throw std::invalid_argument("subconfig is required for model_type=" +
                                            model_type_str);
            }
            const std::string input_str =
                toLower(subconfig_node["input"].as<std::string>());
            const std::string post_str =
                toLower(subconfig_node["post"].as<std::string>());
            if (input_type_map.find(input_str) == input_type_map.end()) {
                throw std::invalid_argument("Unsupported subconfig.input: " + input_str);
            }
            if (post_type_map.find(post_str) == post_type_map.end()) {
                throw std::invalid_argument("Unsupported subconfig.post: " + post_str);
            }
            ms.input_type = input_type_map[input_str];
            ms.post_type = post_type_map[post_str];
        }
